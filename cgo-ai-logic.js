...
export function decide(caseData, policy={}, knowledge=null){
  ...
  return {...evaluation,decision:"AUTO_ALLOWED",reason:"AUTO_APPLY_READY"};
}
...
export function buildAction(caseData, authorization){
  ...
  if(auth.decision==="AUTO_ALLOWED") return {
    action:"AUTO_APPLY_INTENT", executable:true,
  ...
  if(auth.decision==="USER_CONFIRMATION_REQUIRED") return {
    action:"USER_CONFIRMATION_PATCH_AND_EXECUTE", executable:false,
    authorizationId:auth.authorizationId||null, caseId:c.caseId,
