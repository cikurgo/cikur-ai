 ...
 * Binds the internal CGO brain to the existing BCGO and Customer application.
 * No external AI/API. No source mutation. Evidence must be grounded in live internal state.
 */
...
let customerChatBootPromise = null;

async function ensureCustomerChatEngine() {
  ...
  return customerChatBootPromise;
}

async function liveChat(question, options = {}) {
  const cgo = await ensureCustomerChatEngine();
  if (!cgo?.chatAsync) throw new Error("CGO_CUSTOMER_CHAT_UNAVAILABLE");
  const result = await cgo.chatAsync(question, options);
  const text = typeof result === "string" ? result : String(result?.text || result?.response || result?.message || "");
  return { ok: true, text, response: text, engine: "CGO_CUSTOMER_NATURAL_REASONING", conversation: clone(result) };
}
...
function numberSource(source) {
  ...
  return text.split(/\r?\n/).map((line, index) => ({ line:index + 1, code:line }));
}

function sourceRangeForText(source, needle) {
  const lines = numberSource(source);
  const target = String(needle || "").trim();
  if (!target) return [];
  const compact = target.split(/\r?\n/).map(x => x.trim()).filter(Boolean);
  if (!compact.length) return [];
  const first = compact[0];
  const hits = [];
  for (let i=0; i<lines.length; i++) {
    if (!lines[i].code.includes(first)) continue;
    let ok = true;
    for (let j=1; j<compact.length; j++) {
      if (!lines[i+j] || !lines[i+j].code.includes(compact[j])) { ok=false; break; }
    }
    if (ok) hits.push({ startLine:lines[i].line, endLine:lines[Math.min(lines.length-1,i+compact.length-1)].line });
...
async function readExactSourceForWorkbench(file) {
  ...
  const source = await response.text();
  if (!source.trim()) throw new Error("SOURCE_EMPTY");
  const fingerprint = Core.contentFingerprint(source);
  return {
    file:target,
    source,
    fingerprint,
    lines:numberSource(source).length,
    bytes:new TextEncoder().encode(source).length,
    readAt:Date.now()
  };
}

function emitCodeWorkbench(payload) {
  try {
    window.dispatchEvent(new CustomEvent("cgo-exact-source-viewer", { detail:clone(payload) }));
  } catch {}
}
...
async function openCodeWorkbench(file, context = {}) {
  ...
  try {
    ...
    const payload = {
      version:"1.1.0-FULL-SOURCE-EXACT-SOURCE-VIEWER",
      status:"SOURCE_READ",
      file:target,
      original:{
      ...
      sourceIntegrity:{
        liveFingerprint:original.fingerprint,
        candidateOriginalFingerprint:exact?.fingerprint || exact?.contentFingerprint || null,
        match:!exact || !exact.fingerprint ? null : String(exact.fingerprint) === String(original.fingerprint)
      }
    };
    emitCodeWorkbench(payload);
    return payload;
  } catch (error) {
    const payload={version:"1.0.0-EXACT-SOURCE-VIEWER",status:"SOURCE_READ_FAILED",file:target,error:String(error?.message||error)};
    emitCodeWorkbench(payload);
    return payload;
  }
}
...
function isSystemChatRequest(raw, analysis = {}) {
  const q = String(raw || "").toLowerCase();
  if (analysis && ["availability", "order", "location", "action"].includes(analysis.intent)) return false;
...
function setPresenceQueryResolver(resolver) {
  presenceQueryResolver = typeof resolver === "function" ? resolver : null;
  return { ok:true, installed:Boolean(presenceQueryResolver) };
}

function setCustomerLocationResolver(resolver) {
  customerLocationResolver = typeof resolver === "function" ? resolver : null;
  return { ok:true, installed:Boolean(customerLocationResolver) };
}

function getCustomerLocation(options = {}) {
  if (customerLocationResolver) return customerLocationResolver(options);
  return null;
}

async function findNearbyAgents(request = {}) {
  if (presenceQueryResolver) {
    const result = await presenceQueryResolver(request);
    return result || { ok:false, status:"unknown", verified:false, items:[], count:0, reason:"EMPTY_PRESENCE_RESULT" };
  }
  return { ok:false, status:"unknown", verified:false, items:[], count:0, reason:"PRESENCE_RESOLVER_NOT_CONNECTED" };
}

function getPresenceContract() {
  return Object.freeze({ scope:"CUSTOMER_GEO_SCOPED", source:"BCGO_STATE.agentPresence", freshnessMs:180000, exposesCoordinatesToCustomer:false, unknownWhenUnverified:true });
}
...
function chatAnswer(question = {}) {
  ...
  const gapRequested = /\b(kontrak|contract|kurang|missing|seharusnya|gap|tidak ada|belum ada)\b/.test(q);

  if (gapRequested && (requestedFile || state?.sourceScan?.exploration)) {
    const scope = contractGapSummary(state, requestedFile || null);
    if (!scope.scoped.length) return requestedFile ? `Saya sudah cek kandidat contract gap untuk ${requestedFile}. Saat ini tidak ada gap yang terbukti dari heuristik scanner. Statusnya NO_GAP_PROVEN, bukan klaim file sempurna.` : `Contract Gap Engine mencatat ${scope.all.length} kandidat gap. Saya hanya akan menyebutnya sebagai temuan setelah evidence source mendukungnya.`;
    const lines=scope.scoped.slice(0,6).map(g=>`${g.severity} ${g.type} @ ${g.file}:${g.line||'-'} — ${g.message}`).join(' | ');
    return `Saya menemukan ${scope.scoped.length} kandidat contract gap untuk ${requestedFile || 'sistem'}. Ini masih bukti untuk verifikasi, bukan root cause final: ${lines}`;
  }
  ...
  if (isRepairCommand) {
    ...
    if (result && typeof result === "object") {
      ...
      const repairCase = result.caseId ? runtime.getCase(result.caseId) : null;
      if (repairCase?.exactSource?.originalCode || repairCase?.exactSource?.proposedCode) {
        void openCodeWorkbench(targetFile, {
          command:raw,
          caseId:result.caseId,
          exactSource:repairCase.exactSource,
          proof:repairCase.precisionGate,
          finding:repairCase.rootCause || null,
          explanation:repairCase.rootCause?.statement || null
        });
      }
      return result.text;
    }
  ...
  const explicitFile = requestedFile || lastChatContext.files[0] || null;
  if (codeInspectionIntent && explicitFile && !isRepairCommand) {
    void openCodeWorkbench(explicitFile, { command:raw, caseId:lastChatCaseId });
    return `Baik. Saya buka source aktual ${explicitFile} sekarang dan menampilkannya di Exact Source Viewer sebagai FULL SOURCE dengan nomor baris. Saya tidak akan mengarang bagian AFTER; candidate hanya muncul bila memang sudah memiliki proof/source yang sah.`;
  }
...
function compatibleSnapshot(caseId, signal = "LIVE_TELEMETRY", caseOverride = null) {
  ...
  const evaluation = Logic.evaluate(c, INTERNAL_CGO_POLICY, knowledge);
...
export function install() {
  return {
    ...
    /*
     * IMPORTANT: Customer CGO calls brain.ask() from inside its own
     * chat pipeline. brain.ask() MUST therefore be synchronous and MUST
     * NOT call CGO.chatAsync() again. The previous implementation created
     * a recursive Customer -> Brain -> Customer loop and returned a Promise
     * where the gateway expected a response string.
     *
     * The browser brain is the system-reader here: it answers from the
    ...
    ask(question, options = {}) {
      try {
        ...
        if (!result.handled) return { ok:true, handled:false, text:null, response:null, engine:"CGO_INTERNAL_BRAIN", version:VERSION, reason:result.reason };
        return { ok:true, handled:true, text:result.text, response:result.response, engine:"CGO_INTERNAL_BRAIN", version:VERSION, evidenceBound:true };
      } catch (error) {
        return { ok:false, handled:false, text:null, response:null, engine:"CGO_INTERNAL_BRAIN", error:String(error?.message || error) };
      }
    },
    setPresenceQueryResolver,
    setCustomerLocationResolver,
    getCustomerLocation,
    findNearbyAgents,
    getPresenceContract,
    capabilities() {
    ...
    runtimeStatus() { return { runtime: "INTERNAL_APPLICATION", lastChatCaseId, latest: clone(latest) }; },
