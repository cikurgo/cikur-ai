...
export const CASE_TRANSITIONS = {
  ...
  CANDIDATE_READY:["APPLICATION_ACTION_REVIEW","USER_CONFIRMATION","APPLYING_CHANGE","INVESTIGATING"],
  APPLICATION_ACTION_REVIEW:["USER_CONFIRMATION","APPLYING_CHANGE","INVESTIGATION_BLOCKED"],
  USER_CONFIRMATION:["APPLYING_CHANGE","INVESTIGATION_BLOCKED"],
  APPLYING_CHANGE:["VERIFYING_RESULT","RESULT_VERIFICATION_FAILED"],
  VERIFYING_RESULT:["RESOLVED","REOPENED","RESULT_VERIFICATION_FAILED"],
  RESULT_VERIFICATION_FAILED:["REOPENED","INVESTIGATING"],
...
export function buildActionPlan(caseData, authorization) {
  ...
  const action = !verified ? "INVESTIGATE" :
    decision==="AUTO_ALLOWED" ? "AUTO_APPLY_INTENT" :
    decision==="USER_CONFIRMATION_REQUIRED" ? "USER_CONFIRMATION_PATCH_AND_EXECUTE" :
    "BLOCKED";
  c.actionPlan = {
    planId:id("plan"),
    action,
