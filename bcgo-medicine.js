...
import { install as installBridge } from "./cgo-bcgo-bridge.js?v=20260913-medicine5";
...
function team(type,p={}){const packet={id:id(type),bridge:BRIDGE.channel,from:"MEDICINE",type,version:VERSION,at:Date.now(),...clone(p)};try{TEAM?.postMessage(packet);}catch{};try{localStorage.setItem(`${BRIDGE.channel}_EVENT`,JSON.stringify(packet));}catch{};emit(type,{packet});return packet;}
...
function simulate(src,r){const n=(src.match(new RegExp(r.before.replace(/[.*+?^${}()|[\\]\\]/g,"\\$&”),"g"))||[]).length;if(n!==1)return null;let out=r.operation==="REPLACE_EXACT"?src.replace(r.before,r.after):src.replace(r.before,r.before+r.after);if(out===src)return null;return {sourceAfter:out,beforeFingerprint:fingerprint(src),afterFingerprint:fingerprint(out)};}
...
function ingest(state){if(!state)return;S.bcgo=clone(state);S.scan=state.sourceScan||null;S.status="BCGO_LIVE";const rev=Number(S.scan?.rescanGeneration||state.cycle||0);if(rev!==S.lastRevision){S.lastRevision=rev;const findings=[...(S.scan?.findings||[]),...(S.scan?.crossFileFindings||[])].filter(f=>String(f.severity||"").toUpperCase()!=="INFO");for(const f of findings.slice(0,12)){const c=makeCase(f);if(c&&!S.active)S.active=c;if(c&&S.authorized)void investigate(c,"BCGO_NEW_SCAN");}}
 const active=Array.isArray(state.activeCases)?state.activeCases:[];for(const bc of active.slice(0,10)){const f=bc.evidence||bc;const c=makeCase({file:bc.target||bc.file||f.file,type:f.type||"BCGO_ACTIVE_CASE",message:f.message||bc.message||"Active BCGO case",proofStatus:f.proofStatus});if(c){c.bcgoCaseId=bc.id||null;c.evidence.push(clone(f));S.active=c;if(S.authorized&&c.lastInvestigated!==rev)void investigate(c,"BCGO_ACTIVE_CASE");}}
 emit("BCGO_STATE_RECEIVED",{cycle:state.cycle,revision:rev,active:state.metrics?.active||0,total:state.metrics?.total||0});}
...
BRIDGE.onState(ingest);BRIDGE.onTeamReport(handleTeam);TEAM?.addEventListener("message",e=>handleTeam(e.data));window.addEventListener("storage",e=>{if(e.key===`${BRIDGE.channel}_EVENT`&&e.newValue){try{handleTeam(JSON.parse(e.newValue));}catch{}}});
...
window.BCGOMedicine=Object.freeze({version:VERSION,getState:()=>clone(S),getActiveCase:()=>clone(S.active),refresh:()=>ingest(BRIDGE.getState()||window.BCGO_STATE),investigate:(caseId)=>{const c=S.cases.find(x=>x.id===caseId);return investigate(c,"HUMAN_REQUEST")},getCases:()=>clone(S.cases)});
boot();render();
