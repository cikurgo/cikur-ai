...
/*
 * ================================================================
 * CAPTAIN LIFECYCLE BRIDGE
 * ================================================================
 * Medicine retains its local diagnostic status for compatibility, but the
 * canonical case lifecycle is now projected into the 19-file CGO Captain
...
const MEDICINE_TO_CAPTAIN_STATE = Object.freeze({
  ...
  VERIFIED_DIAGNOSIS:"SOURCE_VERIFIED",
  INVESTIGATION_BLOCKED:"INVESTIGATION_BLOCKED",
  READY_FOR_HUMAN_APPROVAL:"HUMAN_APPROVAL",
  READY_FOR_PATCH:"HUMAN_APPROVAL",
  PATCH_APPLIED:"VALIDATING",
  PATCH_FAILED:"VALIDATION_FAILED",
  VALIDATION_FAILED:"VALIDATION_FAILED",
  INTERNAL_VERIFIED_PENDING_DEPLOYMENT:"VALIDATING",
  VERIFIED_FIXED:"RESOLVED"
});

const CAPTAIN_ADVANCE_PATHS = Object.freeze({
  SOURCE_VERIFIED:["VERIFYING","ROOT_CAUSE_VERIFIED","SOURCE_VERIFIED"],
  HUMAN_APPROVAL:["VERIFYING","ROOT_CAUSE_VERIFIED","SOURCE_VERIFIED","CANDIDATE_READY","EXECUTOR_REVIEW","HUMAN_APPROVAL"],
  EXECUTING:["VERIFYING","ROOT_CAUSE_VERIFIED","SOURCE_VERIFIED","CANDIDATE_READY","EXECUTOR_REVIEW","HUMAN_APPROVAL","EXECUTING"],
  VALIDATING:["VERIFYING","ROOT_CAUSE_VERIFIED","SOURCE_VERIFIED","CANDIDATE_READY","EXECUTOR_REVIEW","HUMAN_APPROVAL","EXECUTING","VALIDATING"],
  VALIDATION_FAILED:["VERIFYING","ROOT_CAUSE_VERIFIED","SOURCE_VERIFIED","CANDIDATE_READY","EXECUTOR_REVIEW","HUMAN_APPROVAL","EXECUTING","VALIDATION_FAILED"],
  RESOLVED:["VERIFYING","ROOT_CAUSE_VERIFIED","SOURCE_VERIFIED","CANDIDATE_READY","EXECUTOR_REVIEW","HUMAN_APPROVAL","EXECUTING","VALIDATING","RESOLVED"]
});

function ensureCaptainCase(c){
  if(!c?.id) return null;
  const existing=CAPTAIN_RUNTIME.getCase(c.id);
  if(existing) return existing;
  return CAPTAIN_RUNTIME.detect({
    caseId:c.id,
    target:c.source || c.rootCauseFile || null,
    symptom:c.signature || c.diagnosis?.title || null,
    error:c.signature || null,
    severity:c.diagnosis?.severity || "UNKNOWN",
    source:"MEDICINE",
    observedAt:c.createdAt || now()
  });
}
...
function receiveExecutionResult(packet) {
  ...
  if (result.status === "SUCCESS") {
    setMedicineCaseStatus(c, "PATCH_APPLIED");
    ...
    setMedicineCaseStatus(c, "PATCH_FAILED");
...
async function createLocalSourceFindingCase(finding, sources, scan) {
  ...
  if (!c) {
    ...
    ensureCaptainCase(c);
    projectCaptainState(c,c.status);
  ...
  if (candidate.ready && candidate.operation) {
    ...
    plan.precisionGate=true;plan.status='PROPOSED';plan.blockReason=null;c.rootCauseStatus='CONFIRMED_ORIGINAL_TARGET';setMedicineCaseStatus(c, 'VERIFIED_DIAGNOSIS');
  } else {
    plan.precisionGate=false;plan.status='PATCH_REQUIRES_REVIEW';plan.blockReason=`${candidate.reason || 'SOURCE_FINDING_REQUIRES_DEEPER_REVIEW'} — Medicine tidak mengarang patch.`;setMedicineCaseStatus(c, 'INVESTIGATION_BLOCKED');
  }
...
async function createCrossFileCase(finding, sources, scan) {
  ...
  if (candidate.ready) {
    ...
    setMedicineCaseStatus(c, "VERIFIED_DIAGNOSIS");
  } else {
...
function buildInvestigationDecision(c, v, plan, context = {}) {
  ...
  if (context.internalReasoning?.precisionGate === false) blockers.push(...(context.internalReasoning?.blockers || ["INTERNAL_COGNITION_BLOCKED"]));
  const unique = [...new Set(blockers.filter(Boolean))];
  const ready = plan?.precisionGate === true && context.internalReasoning?.precisionGate !== false;
  ...
  return {
    ...
    aiDirective: context.internalReasoning?.nextEvidence || context.internalReasoning?.directive || null,
...
function runInternalCognition(c, v, plan, context = {}) {
  const evidence = Array.isArray(v?.sourceEvidence) ? v.sourceEvidence : [];
  const contradictions = evidence.filter(e =>
    String(e?.status || "").toUpperCase() === "CONTRADICTORY" ||
    String(e?.type || "").toUpperCase().includes("CONTRADICT")
  );

  const cognitionEvidence = evidence.map((e, index) => ({
    id: e?.id || `MED-EV-${index + 1}`,
    claim: e?.claim || e?.detail || e?.message || null,
    source: e?.source || e?.file || plan?.rootCauseFile || c?.source || null,
    status: String(e?.status || "UNVERIFIED").toUpperCase() === "VERIFIED" ? "VERIFIED" : "UNVERIFIED",
    exact: e?.exact === true || e?.evidenceStrength === "HIGH",
    strength: Number.isFinite(e?.strength) ? e.strength : (e?.evidenceStrength === "HIGH" ? 0.9 : 0.5),
    metadata: e?.metadata || {}
  }));
  ...
  const cognition = InternalCognition.deliberate({
  ...
  try {
    const investigationCase = {
      caseId:c?.id || null,
      target:plan?.rootCauseFile || c?.source || v?.target || null,
      evidence:cognitionEvidence,
      hypotheses:Array.isArray(plan?.candidates) ? plan.candidates : [],
      rootCause:plan?.rootCauseStatus && plan.rootCauseStatus !== "UNPROVEN"
        ? { statement:plan.rootCauseStatus }
        : null,
      exactSource:plan?.sourceEvidence?.some(e => e?.evidenceStrength === "HIGH") ? {} : null
    };
    investigation = InternalInvestigator.createInvestigation(investigationCase, context.knowledge || {});
    nextEvidence = InternalInvestigator.nextProbe(investigation, investigationCase, context.knowledge || {});
  } catch (error) {
    nextEvidence = { type:"INVESTIGATION_ENGINE_ERROR", reason:String(error?.message || error), score:0 };
  }

  const blockers = [];
  if (contradictions.length) blockers.push("CONTRADICTORY_EVIDENCE");
  if (cognition.unknowns.length) blockers.push("EVIDENCE_MISSING");
  if (!plan?.rootCauseFile || plan?.rootCauseStatus === "UNPROVEN") blockers.push("ROOT_CAUSE_UNVERIFIED");
  if (!plan?.sourceEvidence?.some(e => e?.evidenceStrength === "HIGH")) blockers.push("EXACT_HIGH_EVIDENCE_MISSING");

  const result = {
    version:`INTERNAL-${InternalCognition.VERSION}+${InternalInvestigator.VERSION}`,
    classification:cognition.conclusion,
    precisionGate:{pass:blockers.length === 0 && proofComplete, blockers:[...new Set(blockers)]},
    evidence:cognition.facts,
    hypotheses:Array.isArray(plan?.candidates) ? plan.candidates : [],
    selectedHypothesisId:null,
    investigation,
    operationalInvestigation:{
      evidenceRequests:nextEvidence ? [nextEvidence] : [],
      nextEvidence
    },
  ...
  S.aiCore={
    ...
    precisionGate:result.precisionGate.pass,
    blockers:result.precisionGate.blockers,
    evidenceCount:result.evidence.length,
    hypothesisCount:result.hypotheses.length,
    selectedHypothesisId:null,
    investigation:result.investigation,
    operationalInvestigation:result.operationalInvestigation,
    lastAt:now()
  };
...
async function verifyWithMedicine(targetFile = null, context = {}) {
  ...
  try {
    ...
    if (c) {
      ...
      // Internal CGO cognition reviews the completed Medicine evidence surface.
      // It never becomes the proof authority; Medicine evidence + Captain/Guardian remain authoritative.
      const aiReasoning = runInternalCognition(c, v, plan, context);
      ...
      // Internal cognition is advisory. It can block on contradictions/missing evidence,
      // but it never replaces Medicine proof or Captain/Guardian authority.
      const aiHardBlocked = new Set(["CONTRADICTORY_EVIDENCE","EVIDENCE_MISSING"]);
      ...
      const decision = buildInvestigationDecision(c, v, plan, { internalReasoning:{...aiReasoning?.precisionGate, pass:aiCompatible, investigation:aiReasoning?.investigation || null, operationalInvestigation:aiReasoning?.operationalInvestigation || null, nextEvidence:aiReasoning?.operationalInvestigation?.nextEvidence || null}, trigger:context.autoRecovery ? "auto_recovery" : "verification" });
...
async function validateAfterPatch(caseId) {
  ...
  S.validation = v;
  c.validation = v;
  setMedicineCaseStatus(c, v.status);
...
async function rejectTreatment(caseId, reason = "Treatment ditolak oleh manusia.") {
  const c = S.cases.find(x => x.id === caseId);
  if (!c) throw new Error("Case tidak ditemukan");
  setMedicineCaseStatus(c, "REJECTED");
  c.rejectedAt = now();
...
async function publishExecutionApproval(c, proposal) {
  ...
  try {
    captainAuthorization = CAPTAIN_RUNTIME.authorizeExecution(c.id, {
      ...
      file: op.file,
      operation: op.type,
      sourceFingerprint: request.expectedFingerprint || review.sourceFingerprint || review.beforeFingerprint || null,
...
async function approveTreatment(caseId) {
  ...
  setMedicineCaseStatus(c, "READY_FOR_PATCH");
  c.approvedAt = now();
  ...
  const executionApproval = await publishExecutionApproval(c, c.patchProposal);
...
function ingestBCGOActiveCases(activeCases, packet = {}) {
  ...
  for (const bcgoCase of activeCases.slice(0,20)) {
    ...
    S.cases.unshift(c); S.cases=S.cases.slice(0,100); S.activeCase=c; ensureCaptainCase(c); projectCaptainState(c,c.status); emit("bcgo_case_handoff",{case:c,source:"BCGO_STATE",handoff:"READY_FOR_MEDICINE"}); queueAutoInvestigation(c,"bcgo_case_handoff");
  }
}
...
const API = {
  ...
  getCaptainCase:(caseId) => CAPTAIN_RUNTIME.getCase(caseId),
  getCaptainEvidenceLedger:(caseId) => CAPTAIN_RUNTIME.getEvidenceLedger(caseId),
  verifyCaptainEvidenceLedger:() => CAPTAIN_RUNTIME.verifyEvidenceLedger(),
