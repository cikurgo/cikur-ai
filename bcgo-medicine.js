...
function buildRepairPlan(c, verification) {
  return {
    ...
    rootCauseFile:verification?.rootCauseFile || null,
...
async function resolveRootCause(c) {
  ...
  // A target is never promoted to root cause merely because it was the
  // telemetry target. Prefer a different file when it has equally/stronger
  // exact evidence; otherwise keep the root cause unproven.
  const high = candidates.filter(x => x.evidenceStrength === "HIGH");
  const best = high.find(x => normalizeFile(x.file) !== original) || high[0] || null;
