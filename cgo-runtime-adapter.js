...
/*
 * BCGO-facing install bridge.
 * Keeps CGO as the decision/orchestration brain while exposing a natural
 * internal-system query surface and the existing action-chain contracts.
 * Read operations are evidence-first; no external AI/API is used.
 */
export function install(options = {}) {
  ...
  async function readInternalDataSource(file = "data-cgo.html") {
    const target = normalizeFile(file) || "data-cgo.html";
    ...
    if (!response.ok) throw new Error(`INTERNAL_SOURCE_READ_HTTP_${response.status}:${target}`);
    const source = await response.text();
    if (!source.trim()) throw new Error(`INTERNAL_SOURCE_EMPTY:${target}`);
  ...
  function extractCollections(source) {
    const found = [];
    const patterns = [
      /collection\s*\(\s*db\s*,\s*["'`]([^"'`]+)["'`]\s*\)/g,
      /collection\s*\(\s*[^,]+\s*,\s*["'`]([^"'`]+)["'`]\s*\)/g
    ];
    for (const re of patterns) {
      let m;
      while ((m = re.exec(source))) if (!found.includes(m[1])) found.push(m[1]);
    }
    return found.slice(0, 20);
  }

  async function queryInternalData(request = {}) {
    const sourceInfo = await readInternalDataSource(request.file || "data-cgo.html");
    const collections = extractCollections(sourceInfo.source);
    if (!collections.length) {
      return {
        ok: false,
        verified: false,
        file: sourceInfo.file,
        reason: "NO_LITERAL_FIRESTORE_COLLECTION_FOUND",
        collections: [],
        evidence: { source: sourceInfo.file, method: "SOURCE_COLLECTION_EXTRACTION" }
      };
    }
    const results = [];
    for (const path of collections) {
      try {
        ...
        results.push({ collection: path, count: snap.size, verified: true });
      } catch (error) {
        results.push({ collection: path, count: null, verified: false, error: String(error?.message || error) });
      }
    }
    return {
      ok: results.some(x => x.verified),
      verified: results.some(x => x.verified),
      file: sourceInfo.file,
      collections: results,
      evidence: {
        source: sourceInfo.file,
        method: "SOURCE_DRIVEN_FIRESTORE_READ",
  ...
  async function answer(question = "") {
    ...
    const active = Array.isArray(st.activeCases) ? st.activeCases : [];
    const lower = q.toLowerCase();
    let result;

    if (/data-cgo|customer|pelanggan|daftar|registrasi/.test(lower)) {
      try {
        const data = await queryInternalData({ file: "data-cgo.html" });
        if (data.verified) {
          const readable = data.collections
            .filter(x => x.verified)
            .map(x => `${x.collection}: ${x.count} data`)
            .join(", ");
          result = {
            text: `Baik, saya sudah cek sumber yang dipakai data-cgo.html. Data yang berhasil saya verifikasi sekarang: ${readable || "belum ada data yang bisa dibaca"}. Saya menyampaikan angka ini berdasarkan hasil baca internal saat ini.`,
            action: "QUERY_INTERNAL_DATA",
            evidence: data
          };
        } else {
          result = {
            text: "Saya sudah membuka data-cgo.html, tetapi belum menemukan sumber koleksi Firestore yang bisa saya buktikan dari source tersebut. Jadi saya tidak akan mengarang jumlah pendaftar.",
            action: "QUERY_INTERNAL_DATA_BLOCKED",
            evidence: data
          };
        }
      } catch (error) {
        result = {
          text: `Saya sedang mencoba membaca sumber data internal data-cgo.html, tetapi pembacaan belum berhasil: ${String(error?.message || error)}. Saya tidak akan mengganti hasil itu dengan angka perkiraan.`,
          action: "QUERY_INTERNAL_DATA_ERROR"
        };
      }
    } else if (/aman|status sistem|kondisi sistem/.test(lower)) {
      result = {
        text: active.length
          ? `Saya belum menyebut sistem sepenuhnya aman. Saat ini saya melihat ${active.length} kasus/anomali aktif dan masih saya pantau berdasarkan telemetry.`
          : `Untuk kondisi yang bisa saya buktikan sekarang, belum ada anomali aktif yang saya terima. Saya tetap memantau ${metrics.total ?? "sejumlah"} organ dan telemetry real-time.`,
        action: "SYSTEM_STATUS"
      };
    } else if (/hubungan|relasi|terhubung|dependency/.test(lower)) {
      const relations = Array.isArray(st.sourceScan?.relations) ? st.sourceScan.relations : [];
      const pairs = relations.slice(0, 8).map(r => `${normalizeFile(r.sourceFile || r.from || r.file) || "?"} × ${normalizeFile(r.targetFile || r.to || r.relatedFile) || "?"}`);
      result = {
        text: pairs.length ? `Saya menemukan ${relations.length} relasi source yang terbukti. Contohnya: ${pairs.join(", ")}.` : "Saya belum memiliki pasangan relasi source yang cukup untuk menyimpulkan hubungan file.",
        action: "RELATION_LOOKUP"
      };
    } else if (/perbaiki|perbaikan|patch|tulis|write|execute|eksekusi/.test(lower)) {
      result = {
        text: "Saya memahami permintaan tindakan. Saya akan menjaga urutannya: pahami target → kumpulkan evidence → verifikasi root cause dan source → susun action plan → baru jalankan jalur WRITE/PATCH/EXECUTE sesuai otorisasi yang berlaku. Saya tidak akan melompati bukti.",
        action: "ACTION_PLAN_REQUEST"
      };
    } else {
      const previous = conversation.length ? conversation[conversation.length - 1]?.text : null;
      result = {
        text: previous
          ? `Saya masih membawa konteks percakapan sebelumnya. Saya akan menghubungkan pertanyaan ini dengan evidence dan state internal yang terakhir saya pegang, bukan memulai dari nol.`
          : `Saya pahami. Saya akan menggunakan state, saraf file, evidence, dan konteks percakapan yang sedang hidup sebelum menyimpulkan sesuatu. Saat ini target saya ${st.targetCell || "belum ditentukan"}.`,
        action: "REASON_FROM_INTERNAL_STATE"
      };
    }
    conversation.push({ role: "user", text: q, at: Date.now() });
    conversation.push({ role: "cgo", text: result.text, at: Date.now(), action: result.action });
    while (conversation.length > 30) conversation.shift();
    return result;
  }
  ...
  return {
    version: VERSION,
    ingestBCGOState(state = {}) { latestState = structuredClone(state); return runtime.getCase ? { accepted: true } : { accepted: false }; },
    getSnapshot() { return latestState ? structuredClone(latestState) : null; },
    getConversation() { return structuredClone(conversation); },
    async ask(question = "") { const out = await answer(question); return out.text; },
    async queryInternalData(request = {}) { return queryInternalData(request); },
    deliberate(caseId, policy = {}) { return runtime.deliberate(caseId, policy); },
    logic(caseId, policy = {}) { return runtime.logic(caseId, policy); },
    investigate(caseId) { return runtime.investigate(caseId); },
    plan(caseId, policy = {}) { return runtime.plan(caseId, policy); },
    authorize(caseId, policy = {}) { return runtime.authorize(caseId, policy); },
    async execute(caseId, policy = {}) { return runtime.execute(caseId, policy); },
    bindExecutionTarget(target) { return runtime.bindExecutionTarget(target); },
    setExecutor(executor) { return runtime.setExecutor(executor); },
    hasExecutionHand() { return runtime.hasExecutionHand(); },
