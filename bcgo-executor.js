...
import { adminDb } from "./cikur-config.js?v=20260912-real-cgo-bridge2";
...
(() => {
  ...
  function normalize(request) {
    ...
    return {
      ...
      authorizationId: String(request.authorizationId || request.authorization?.authorizationId || "").trim(),
      authorization: request.authorization && typeof request.authorization === "object" ? structuredClone(request.authorization) : null
    };
  }
  ...
  function validateRequest(r, source) {
    ...
    if (r.approval !== "APPROVED") errors.push("EXECUTION_AUTHORIZATION_REQUIRED");
...
function publishInvestigationAck(packet) {
  ...
  async function handleExecutionApproval(packet, source = "BROADCAST_CHANNEL") {
    ...
    const at = Number(packet.at) || 0;
    if (!at || Date.now() - at > REPAIR_CANDIDATE_MAX_AGE) return false;
    if (packet.approval !== "CAPTAIN_AUTHORIZED") return false;
    const captainAuth = packet.authorization;
    if (!captainAuth?.authorizationId || !captainAuth?.binding) return false;
    const requestId = String(packet.requestId || packet.request?.requestId || "").trim();
    ...
    try {
      ...
      const request = {...reviewed.request, approval:"APPROVED", actorUid:packet.approvedBy || reviewed.request?.actorUid || null, authorizationId:captainAuth.authorizationId, authorization:captainAuth};
      let binding;
      try { binding = JSON.parse(captainAuth.binding); } catch { return false; }
  ...
  async function execute(request, sourceText = null) {
    ...
    if (r.approval !== "APPROVED" || !r.authorizationId || !r.authorization?.binding) {
      setStatus(STATUS.WAITING_APPROVAL);
      return reject("CAPTAIN_EXECUTION_AUTHORIZATION_REQUIRED");
    }
    let authorizationBinding;
    try { authorizationBinding = JSON.parse(r.authorization.binding); }
    catch { return reject("INVALID_CAPTAIN_AUTHORIZATION"); }
    const bindingChecks = {
      caseId:r.caseId, proposalId:r.proposalId, planId:r.planId, file:r.file, sourceId:r.sourceId,
      operation:r.operation, beforeFingerprint:core().fingerprint(r.before), proposedFingerprint:core().fingerprint(r.after)
    };
    for (const key of Object.keys(bindingChecks)) {
      if (String(authorizationBinding[key] ?? "") !== String(bindingChecks[key] ?? "")) return reject("CAPTAIN_AUTHORIZATION_BINDING_MISMATCH");
    }
