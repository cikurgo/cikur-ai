...
export function authorizeAction(input={}) {
  ...
  if(risk==="CRITICAL") return {decision:"USER_CONFIRMATION_REQUIRED",risk,reason:"CRITICAL_CHANGE_USER_CONFIRMATION_REQUIRED",policyVersion:policy.version||"1.5"};
  if(risk==="HIGH") return {decision:"USER_CONFIRMATION_REQUIRED",risk,reason:"HIGH_RISK_CHANGE_USER_CONFIRMATION_REQUIRED",policyVersion:policy.version||"1.5"};
  if(policy.allowAutomaticExecution===false) return {decision:"USER_CONFIRMATION_REQUIRED",risk,reason:"POLICY_REQUIRES_USER_CONFIRMATION",policyVersion:policy.version||"1.5"};
