...
(function (window) {
    ...
    const VERSION = "1.3.0-customer-brain-primary-gateway";
    ...
    function askBrainIfApplicable(input, analysis, knowledge, discovery) {
        ...
        if (!text) return null;
        // The Brain is attempted for every conversational turn. It decides
        // whether it has enough internal evidence to answer. It must never
        // manufacture a response merely because it was called.
        if (analysis && ["availability", "order", "location", "action"].includes(analysis.intent)) return null;
        try {
            const method = typeof brain.reasonChat === "function"
                ? brain.reasonChat
                : typeof brain.ask === "function" ? brain.ask : null;
            if (!method) return null;
            const result = method.call(brain, {
                text,
                source: "CGO_CUSTOMER",
                analysis: clone(analysis || {}),
                knowledge: clone(knowledge || {}),
                discovery: clone(discovery || {}),
                conversation: clone(state.messages.slice(-12))
            });
            const answer = typeof result === "string" ? result :
                result && result.handled === true && typeof result.text === "string" ? result.text :
                result && result.handled === true && typeof result.response === "string" ? result.response : null;
    ...
    function getModuleStatus() {
        return {
            ...
            brain: !!getBrainModule() && (typeof getBrainModule().reasonChat === "function" || typeof getBrainModule().ask === "function")
        };
    }
